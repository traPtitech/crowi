...
class Crowi {
  ...
  setupSearcher() {
    ...
    const esUser = this.env.ELASTICSEARCH_USER || null
    const esPassword = this.env.ELASTICSEARCH_PASSWORD || null

    return new Promise((resolve, reject) => {
      if (searcherUri) {
        try {
          this.searcher = new (require(path.join(this.libDir, 'util', 'search')))(this, searcherUri, esUser, esPassword)
